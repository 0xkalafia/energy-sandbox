 ...
 *   src/data/geo/amphoe/<iso>.ts  that province's amphoe, at finer detail
 *
...
/**
 * Amphoe files live one level down, and that is not cosmetic. Vite refuses to
 * transform `import(`./${iso}.ts`)` when the pattern could match its own
 * directory, so with everything flat the lazy load is left untransformed: it
 * works in dev and silently fails to resolve in a production build.
 */
const AMPHOE_OUT = "src/data/geo/amphoe";
...
/**
 * A [lon, lat] that stands for the whole province.
 *
 * The province centroid is the right answer where it lands inside — but a
 * province wrapped around a bay or a neighbour can put its own centroid in
 * someone else's territory, and a solar figure fetched for the wrong place is
 * worse than an approximate one. So it's checked, and falls back to the centre
 * of the largest amphoe, which is inside by construction.
 */
function sampleLonLat(outlineRings, fallbackRing) {
  const round = (p) => p.map((v) => +v.toFixed(4));
  if (outlineRings.length) {
    const biggestOutline = outlineRings[0];
    const c = centroid(biggestOutline);
    if (outlineRings.some((r) => inRing(c, r))) return round(c);
  }
  return fallbackRing ? round(centroid(fallbackRing)) : [0, 0];
}
...
mkdirSync(AMPHOE_OUT, { recursive: true });
...
for (const p of loaded) {
  ...
  index.push({
    ...
    // In degrees, and the centre of the *province*, not of its largest
    // amphoe — anything that asks a service about this place wants a point
    // representative of the whole. Falls back to the amphoe centroid where the
    // province is concave enough that its centroid lands outside it.
    lonLat: sampleLonLat(outlineRings, biggest),
...
`;
  writeFileSync(join(AMPHOE_OUT, `${p.iso}.ts`), body);
 ...
 *
 * The subdirectory in the path is load-bearing: Vite will not transform a
 * variable import whose pattern could match its own directory, and an
 * untransformed one resolves in dev and fails in a production build.
 */
export async function loadAmphoe(iso: string) {
  const mod = await import(\`./amphoe/\${iso}.ts\`);
  return mod.AMPHOE;
}
`,
);
